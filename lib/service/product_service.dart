...
class ProductService {
  ...
  Future<List<FilterProducts>> getProductsByCatName(String catName, {int page = 1, int perPage = 10}) async {
    final headers = await getHeaders();
    final uri = Uri.parse('$baseUrl/catName?catName=$catName&page=$page&perPage=$perPage');
    final response = await http.get(uri, headers: headers);
    return json.decode(response.body);
  }

  Future<List<FilterProducts>> getProductsByCatId(String catId, {int page = 1, int perPage = 10}) async {
    final headers = await getHeaders();
  ...
  Future<List<FilterProducts>> getProductsBySubCatId(String subCatId, {int page = 1, int perPage = 10}) async {
    final headers = await getHeaders();
    final uri = Uri.parse('$baseUrl/subCatId?subCatId=$subCatId&page=$page&perPage=$perPage');
    final response = await http.get(uri, headers: headers);
    return json.decode(response.body);
  }
  ...
  Future<List<FilterProducts>> filterByPrice({
    ...
    final uri = Uri.parse('$baseUrl/filterByPrice').replace(queryParameters: queryParams);
    final response = await http.get(uri, headers: headers);
    return json.decode(response.body);
  }
  ...
  Future<List<FilterProducts>> getProductsByRating({
    ...
    final uri = Uri.parse('$baseUrl/rating').replace(queryParameters: queryParams);
    final response = await http.get(uri, headers: headers);
    return json.decode(response.body);
  }
